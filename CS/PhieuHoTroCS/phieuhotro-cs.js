...
if (typeof auth !== "undefined" && auth && typeof auth.onAuthStateChanged === "function") {
  auth.onAuthStateChanged(async (user) => {
    ...
    try {
      let profile = {};
      const userDoc = db && typeof db.collection === "function"
      ...
      if (userDoc && userDoc.exists) {
        ...
        const legacyDoc = await db.collection("accounts").doc(user.uid).get();
        if (legacyDoc.exists) {
          profile = legacyDoc.data() || {};
        }
      }

      const name = profile.name || profile.fullName || user.displayName || "Học viên";
