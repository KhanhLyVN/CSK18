...
(function () {
  ...
  console.log("Default department:", DEFAULT_DEPARTMENT_CODE);
  console.log("==========================================");
  /* =====================================================
       ICONS
    ===================================================== */
  ...
  function getTicketType(ticket) {
    const category = resolveTicketLabel(
      ticket.ticketCategory || ticket.category || "",
    );
    const issue = resolveTicketLabel(
      ticket.ticketIssue || ticket.issue || ticket.detail || "",
    );
    /*
  ...
  function getTicketTitle(ticket) {
    return (
      ticket.title || ticket.subject || ticket.question || "Không có tiêu đề"
    );
  }
  function getStudentName(ticket) {
    return ticket.name || ticket.studentName || ticket.fullName || "Học viên";
  }
  ...
  function formatTicketDate(ticket) {
    ...
    if (ticket.createdAt) {
      const millis = getTimestampMillis(ticket.createdAt);
      if (millis) {
  ...
  if (todayElement) {
    todayElement.textContent = todayLabel();
  }
  /* =====================================================
       GREETING
    ===================================================== */
  ...
  const greetingElement = document.getElementById("greetingLine");
  if (greetingElement) {
    greetingElement.textContent = `${greetingByHour()} — đây là tổng quan các yêu cầu hỗ trợ từ học viên hiện có trên hệ thống.`;
  }
  /* =====================================================
       CAMPUS
    ===================================================== */
  ...
  function resolveDepartmentCode(ticket) {
    ...
    if (ticket.department) {
      if (typeof ticket.department === "object") {
        if (ticket.department.code) {
          return normalizeDepartmentCode(ticket.department.code);
        }
      }
      if (typeof ticket.department === "string") {
        const department = normalizeDepartmentCode(ticket.department);
    ...
    if (ticket.ticketType) {
      const ticketType = String(ticket.ticketType).trim();
      if (DEPARTMENT_ROUTING[ticketType]) {
        return normalizeDepartmentCode(DEPARTMENT_ROUTING[ticketType]);
      }
      if (ticketType.toLowerCase().startsWith("system")) {
        return "IT";
      }
    }
    /*
    ...
    if (ticket.ticketCategory) {
      const category = String(ticket.ticketCategory).trim();
      if (DEPARTMENT_ROUTING[category]) {
        return normalizeDepartmentCode(DEPARTMENT_ROUTING[category]);
      }
    }
    /*
     * 5. Mặc định
     */
    return normalizeDepartmentCode(DEFAULT_DEPARTMENT_CODE);
  }
  /* =====================================================
       TÌM CAMPUS DOCUMENT
    ===================================================== */
  async function findCampusDepartment(campus, departmentCode) {
    const normalizedCampus = normalizeCampus(campus);
    const normalizedCode = normalizeDepartmentCode(departmentCode);
    if (!normalizedCampus) {
      throw new Error("Không xác định được campus của ticket.");
    }
    if (!normalizedCode) {
      throw new Error("Không xác định được departmentCode.");
    }
    console.log("🔎 Tìm campus document:", {
    ...
    const snapshot = await db
      .collection(CAMPUS_COLLECTION)
      .where("campus", "==", normalizedCampus)
      .limit(50)
      .get();
    if (snapshot.empty) {
      throw new Error(
    ...
    const selectedDoc = activeDoc || fallbackDoc;
    if (!selectedDoc) {
      throw new Error(
    ...
    const result = {
      ...
      departmentId: selectedDoc.id,
      departmentCode: normalizeDepartmentCode(data.code || normalizedCode),
      departmentName: data.name || "",
      campus: normalizeCampus(data.campus || normalizedCampus),
      campusId: data.campusId || selectedDoc.id || "",
    ...
    console.log("✅ ĐÃ TÌM THẤY CAMPUS DOCUMENT:", result);
    return result;
  }
  /* =====================================================
       TÌM ASSIGNEE
  ...
  async function findTicketAssignee(ticket) {
    ...
    if (!campus) {
      throw new Error("Ticket chưa có campus.");
    }
    const departmentCode = resolveDepartmentCode(ticket);
    console.log("📌 ROUTING TICKET:", {
      ticket: getTicketNum(ticket),
      campus: campus,
      departmentCode: departmentCode,
      ticketType: ticket.ticketType,
      priority: getTicketPriority(ticket),
    });
    const department = await findCampusDepartment(campus, departmentCode);
    ...
    return {
      ...
      assigneeUid: department.managerUid || "",
      assigneeName: department.managerName || "",
      assigneeEmail: department.managerEmail || "",
    };
  }
  /* =====================================================
       ASSIGN TICKET
  ...
  async function assignTicketIfNeeded(ticket) {
    ...
    try {
      ...
      await db.runTransaction(async (transaction) => {
        ...
        if (!freshDoc.exists) {
          throw new Error("Ticket không còn tồn tại.");
        }
        const freshData = freshDoc.data() || {};
        /*
         * Tab khác đã assign
         */
        if (freshData.departmentId) {
          console.log(`Ticket ${ticketNum} đã được assign trước đó.`);
          return;
        }
        const updateData = {
      ...
      console.error(`❌ Không thể assign ticket ${ticketNum}:`, error);
      return {
        ...ticket,
        assignmentError: error.message || "Không thể phân công ticket",
      };
    }
  }
  /* =====================================================
  ...
  async function repairUnassignedTickets() {
    console.log("🔧 Bắt đầu repair ticket chưa phân công...");
    try {
      const snapshot = await db
        ...
        .get();
      if (snapshot.empty) {
        console.log("Không có ticket active.");
        return;
      }
      const tickets = [];
      snapshot.forEach((docSnap) => {
      ...
      const unassigned = tickets.filter((ticket) => !ticket.departmentId);
      console.log(`📋 Tổng ticket: ${tickets.length}`);
      console.log(`📋 Ticket chưa assign: ${unassigned.length}`);
      ...
      for (const ticket of unassigned) {
        const result = await assignTicketIfNeeded(ticket);
        if (result && (result.departmentId || result.assignmentSkipped)) {
          success++;
        } else {
          failed++;
        }
      ...
      console.error("❌ Lỗi repair ticket:", error);
    }
  }
  /* =====================================================
  ...
  function startUnassignedTicketListener() {
    if (typeof unassignedUnsubscribe === "function") {
      unassignedUnsubscribe();
      unassignedUnsubscribe = null;
    }
    console.log("👀 Bắt đầu theo dõi ticket mới chưa assign...");
    unassignedUnsubscribe = db
      .collection(TICKET_COLLECTION)
      .where("status", "in", ["open", "pending", "in_progress"])
      .onSnapshot(
        async (snapshot) => {
          for (const docSnap of snapshot.docs) {
            ...
            await assignTicketIfNeeded(ticket);
          }
        },
        (error) => {
          console.error("❌ Listener ticket chưa assign lỗi:", error);
        },
      );
  }
  /* =====================================================
  ...
  async function loadCSProfile(uid) {
    try {
      ...
      console.error("❌ Không thể tải hồ sơ CS:", error);
      return null;
    }
  }
  /* =====================================================
  ...
  auth.onAuthStateChanged(async (user) => {
     ...
     */
    if (!user) {
      console.warn("⚠️ Không có CS đăng nhập.");
      if (dashboardUnsubscribe) {
        dashboardUnsubscribe();
        dashboardUnsubscribe = null;
      }
      if (unassignedUnsubscribe) {
        unassignedUnsubscribe();
        unassignedUnsubscribe = null;
      }
  ...
  function renderStats(tickets) {
    ...
    if (statOpen) {
      statOpen.textContent = counts.open + counts.pending;
    }
    if (statProgress) {
  ...
  function renderUnanswered(tickets) {
    ...
    const element = document.getElementById("unansweredList");
    if (!element) {
      return;
    }
    /*
     ...
     * mới lấy 10 ticket.
     */
    const urgentTickets = list.slice(0, 10);
    element.innerHTML = urgentTickets
      .map((ticket) => {
        const ticketNum = getTicketNum(ticket);
        const priority = getTicketPriority(ticket);
        const priorityLabel = getPriorityLabel(ticket);
        return `
                        <a
                        ...
                        >
                            <div class="mi-top">
                                ...
                                <span class="mi-date">
                                    ${escapeHtml(formatTicketDate(ticket))}
                                </span>
                            </div>
                            <div class="mi-title">
                            ...
                            <div class="mi-name">
                                ${escapeHtml(getStudentName(ticket))}
                                ·
                                <strong
                                    style="
                                        color:var(--maroon)
                                    "
                                >
                                    ${escapeHtml(getTicketType(ticket))}
                                </strong>
                                ·
                                <strong
  ...
  function renderTypeBreakdown(tickets) {
    const counts = {};
    tickets.forEach((ticket) => {
      const key = getTicketType(ticket);
      counts[key] = (counts[key] || 0) + 1;
    });
    const entries = Object.entries(counts).sort((a, b) => b[1] - a[1]);
    const max = entries.length ? entries[0][1] : 1;
    const element = document.getElementById("typeBarList");
    if (!element) {
      return;
    }
    if (entries.length === 0) {
  ...
  function renderRecent(tickets) {
    ...
    const element = document.getElementById("recentList");
    if (!element) {
      return;
    }
    /*
  ...
  function goToTicket() {
    ...
    const value = quickFindInput.value.trim();
    if (!value) {
      if (quickFindErr) {
    ...
    window.location.href = `/CS/TicketManagement/cs-ticket.html?ticket=${encodeURIComponent(value)}`;
  }
  if (quickFindBtn) {
  ...
  window.CSDashboard = {
    ...
    getPriorityWeight,
    getPriorityLabel,
    currentUser: () => currentCSUser,
    currentProfile: () => currentCSProfile,
  };
})();
