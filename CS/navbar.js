...
(function () {
  ...
  const icons = {
    ...
    tickets: '<svg viewBox="0 0 24 24" aria-hidden="true"><path d="M21 15a2 2 0 0 1-2 2H7l-4 4V5a2 2 0 0 1 2-2h14a2 2 0 0 1 2 2Z"></path></svg>',
    create: '<svg viewBox="0 0 24 24" aria-hidden="true"><path d="M12 20h9"></path><path d="M16.5 3.5a2.1 2.1 0 0 1 3 3L7 19l-4 1 1-4Z"></path></svg>',
    reports: '<svg viewBox="0 0 24 24" aria-hidden="true"><path d="M3 3v18h18"></path><path d="m7 16 4-5 3 3 5-7"></path></svg>',
  ...
  function resolveContent(app) { return app.querySelector(":scope > main") || app.querySelector(":scope > .main") || app.querySelector(":scope > .page") || app.querySelector(":scope > .wrap") || app.querySelector(":scope > [data-page-content]"); }
  ...
  function menuMarkup() { return links.map(([page, href, label]) => `<a data-nav-page="${page}" href="${href}">${icons[page]}<span>${label}</span></a>`).join(""); }
  function setActive(sidebar) { const page = document.body.dataset.navPage || links.find(([, href]) => window.location.pathname.endsWith(href))?.[0] || ""; sidebar.querySelectorAll("[data-nav-page]").forEach(link => { const active = link.dataset.navPage === page; link.classList.toggle("active", active); if (active) link.setAttribute("aria-current", "page"); else link.removeAttribute("aria-current"); }); }

  function initNavbar() {
    let app = document.querySelector(".app");
    if (!app) {
      const standalone = document.querySelector("body > .wrap, body > [data-page-content]");
    ...
    const closeMobile = () => { layout.classList.remove("is-mobile-open"); overlay.classList.remove("is-visible"); toggle.setAttribute("aria-expanded", "false"); };
    const syncLayout = () => { if (isMobile()) { layout.classList.remove("is-collapsed"); closeMobile(); } else { layout.classList.toggle("is-collapsed", safeGet(COLLAPSE_KEY) === "true"); toggle.setAttribute("aria-expanded", String(!layout.classList.contains("is-collapsed"))); } };
    toggle.addEventListener("click", () => { if (isMobile()) { const opening = !layout.classList.contains("is-mobile-open"); layout.classList.toggle("is-mobile-open", opening); overlay.classList.toggle("is-visible", opening); toggle.setAttribute("aria-expanded", String(opening)); } else { const collapsed = !layout.classList.contains("is-collapsed"); layout.classList.toggle("is-collapsed", collapsed); safeSet(COLLAPSE_KEY, String(collapsed)); toggle.setAttribute("aria-expanded", String(!collapsed)); } });
