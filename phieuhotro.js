const EMAILJS_PUBLIC_KEY  = "dmcYr1M1K9V45Q18B";
  ...
  // tự động điền ngày hiện tại (người dùng vẫn có thể đổi)
  ...
  function updateStub(){
    document.getElementById('livePreviewNum').textContent = ticketNum;
    ...
    document.getElementById('stubCourseBody').textContent = course || '—';

    const title = document.getElementById('fTitle').value.trim();
    document.getElementById('stubTitle').textContent = title || '—';

    const selectedChip = document.querySelector('.chip.active');
    const catLabel = selectedChip ? selectedChip.querySelector('span').textContent : "Chưa chọn loại";
    const catIconKey = selectedChip ? selectedChip.querySelector('input').dataset.icon : "other";
    document.getElementById('stubCat').innerHTML = svgIcon(catIconKey) + `<span>${catLabel}</span>`;
  ...
  fDateEl.addEventListener('input', updateStub);

  updateStub();

  // submit -> gửi trực tiếp qua EmailJS
    ...
    emailjs.send(EMAILJS_SERVICE_ID, EMAILJS_TEMPLATE_ID, templateParams)
      .then(() => {
        document.getElementById('successText').innerHTML =
          `Phiếu <strong>${ticketNum}</strong> — "<em>${title}</em>" đã được gửi qua email cho bộ phận hỗ trợ. Chúng tôi sẽ phản hồi lại <strong>${email}</strong> trong vòng 24–48 giờ làm việc.`;
