document.addEventListener("DOMContentLoaded", () => {
        ...
        (item) => `
          <a class="notification-item" href="/HV/chat-hv/trao-doi-ticket.html?ticket=${encodeURIComponent(item.ticketId)}">
            <div class="notification-item-head">
              <span class="notification-item-code">${escapeHTML(item.ticketNum)}</span>
              <span class="notification-item-time">${escapeHTML(formatDate(item.createdAt || item.updatedAt))}</span>
            </div>
    ...
    if (!tickets.length) {
      recentListEl.innerHTML = `
        <div class="empty-state">
          Bạn chưa có ticket nào. Hãy tạo yêu cầu đầu tiên.
        </div>
      `;
      return;
    }
    recentListEl.innerHTML = tickets
      .slice(0, 5)
      .map((ticket) => {
        ...
        return `
          <div class="recent-item-wrap">
            <a class="recent-item" href="/HV/chat-hv/trao-doi-ticket.html?ticket=${query}">
              <span class="recent-code">${escapeHTML(number)}</span>
              <span class="recent-title-wrap">
                <span class="recent-title" title="${escapeHTML(title)}">${escapeHTML(title)}</span>
                <span class="recent-type">${escapeHTML(type)}</span>
              </span>
              <span class="status-badge ${meta.className}">${meta.label}</span>
              <span class="recent-date">${escapeHTML(firstValue(ticket, "date") || formatDate(ticket.createdAt))}</span>
            </a>
  ...
  if (!database) {
    ...
    if (recentListEl) {
      recentListEl.innerHTML = `
        <div class="empty-state">Chưa cấu hình kết nối dữ liệu.</div>
      `;
    }
    return;
  }
  ...
  if (!authInstance) {
    setText(connectionLabelEl, "Chưa đăng nhập");
    if (recentListEl) {
      recentListEl.innerHTML = `
        <div class="empty-state">Vui lòng đăng nhập để xem ticket.</div>
      `;
    }
    return;
  }
  ...
  authInstance.onAuthStateChanged(async (user) => {
    ...
    if (!user) {
      ...
      if (recentListEl) {
        recentListEl.innerHTML = `
          <div class="empty-state">Vui lòng đăng nhập để xem ticket của bạn.</div>
        `;
      }
      return;
    }
    ...
    ticketQuery.onSnapshot(
      (snapshot) => {
        ...
        console.log(`Đã tải ${tickets.length} ticket của user ${currentUid}`);
      },
      (error) => {
        console.error("Không thể tải ticket của học viên:", error);
        connectionDotEl?.classList.remove("live");
        setText(connectionLabelEl, "Mất kết nối");
        if (recentListEl) {
          recentListEl.innerHTML = `
            <div class="empty-state">
              Không thể tải ticket. Vui lòng kiểm tra kết nối và thử lại.
            </div>
          `;
        }
      },
