...
function loadUsers() {
  try {
    ...
    if (typeof db === "undefined") {
      console.error("Không tìm thấy biến db. Kiểm tra firebase-config.js.");
      setConnection(false, "Không tìm thấy Firestore");
      showTableError("Không tìm thấy kết nối Firestore.");
      return;
    }
    /* Realtime Firestore */
    unsubscribeUsers = db.collection(USER_COLLECTION).onSnapshot(
      (snapshot) => {
        allUsers = [];
        snapshot.forEach((doc) => {
           ...
           * role: cs / customer_success / Customer Success
           */
...
function isCustomerSuccess(data) {
  ...
  /*
   * Nếu hệ thống users hiện tại không có accountType/role/type
   * thì vẫn cho hiển thị. Điều này tránh việc bảng bị 0 hồ sơ.
   */
  return true;
}
...
function initFilters() {
  ...
  if (elements.campusFilter) {
    elements.campusFilter.addEventListener("change", () => {
      currentCampus = elements.campusFilter.value || "all";
      applyFilters();
    });
  }
  if (elements.departmentFilter) {
    elements.departmentFilter.addEventListener("change", () => {
      currentDepartment = elements.departmentFilter.value || "all";
      applyFilters();
    });
  }
...
function populateFilters() {
  if (elements.campusFilter) {
    const campuses = uniqueValues(allUsers.map((user) => getCampus(user)));
    const currentValue = elements.campusFilter.value || "all";
    elements.campusFilter.innerHTML = `<option value="all">Tất cả campus</option>`;
...
function applyFilters() {
  filteredUsers = allUsers.filter((user) => {
    /* Campus */
    if (currentCampus !== "all" && getCampus(user) !== currentCampus) {
      return false;
    }
    /* Department */
...
function renderCampusReport() {
  ...
  if (!filteredUsers.length) {
    elements.campusReport.innerHTML = `<div class="loading">Không có dữ liệu campus</div>`;
    return;
  }
  const counts = {};
  ...
  elements.campusReport.innerHTML = entries
    .map(([campus, count]) => {
...
function renderDepartmentReport() {
  ...
  if (!filteredUsers.length) {
    elements.departmentReport.innerHTML = `<div class="loading">Không có dữ liệu phòng ban</div>`;
    return;
  }
  const counts = {};
  filteredUsers.forEach((user) => {
  ...
  const entries = Object.entries(counts).sort((a, b) => b[1] - a[1]);
  const max = Math.max(...entries.map(([, count]) => count), 1);
  elements.departmentReport.innerHTML = entries
...
function renderActivityTable() {
  ...
  if (!filteredUsers.length) {
    elements.activityBody.innerHTML = `
      <tr>
        <td colspan="6" class="empty-cell">Không tìm thấy tài khoản phù hợp.</td>
      </tr>
    `;
    updateRecordInfo(0);
    return;
  }
  elements.activityBody.innerHTML = filteredUsers
    .map((user) => {
      ...
      return `
        <tr>
          ...
          <td><span class="campus-text">${escapeHTML(campus)}</span></td>
          <td><span class="department-text">${escapeHTML(department)}</span></td>
          <td>${statusHTML}</td>
          <td><span class="last-active">${escapeHTML(lastLogin)}</span></td>
          <td><span class="joined">${escapeHTML(createdAt)}</span></td>
        </tr>
      `;
    })
    .join("");
  updateRecordInfo(filteredUsers.length);
}
...
function clearFilters() {
  ...
  if (elements.periodFilter) elements.periodFilter.value = DEFAULT_PERIOD;
  if (elements.campusFilter) elements.campusFilter.value = "all";
      ...
      db.collection(USER_COLLECTION)
        ...
        .then((snapshot) => {
          ...
          const campus =
            ...
            data.codeCampus ||
            "";
          setAdminDisplay(name, email, campus);
        })
        .catch((error) => {
...
function setAdminDisplay(name, email, campus = "") {
...
/* =========================================================
   ...
   Hiển thị theo logic của trang System: Tên Admin Campus
========================================================= */
(() => {
  "use strict";
  ...
  function renderAdmin(name, campus, email) {
    ...
    if (adminCampus) {
      ...
      adminCampus.title = finalCampus ? `Campus ${finalCampus}` : "";
    }
    if (adminAvatar) {
...
function initNotifications() {
  ...
  elements.noticeBtn.addEventListener("click", (event) => {
