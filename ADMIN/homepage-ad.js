...
(() => {
  ...
  function normalizeStatus(item) {
    const rawStatus = String(
      valueOf(item, "status", "accountStatus", "state") || "",
    )
      .toLowerCase()
      .trim();
    if (
      ["active", "activated", "đang hoạt động", "online"].includes(rawStatus)
    ) {
      return "active";
    }
    if (["away", "tạm vắng", "offline"].includes(rawStatus)) {
      return "away";
    }
    if (
      ["pending", "inactive", "chưa kích hoạt", "disabled"].includes(rawStatus)
    ) {
      return "pending";
    }
    /*
  ...
  function statusLabel(status) {
    ...
    return labels[status] || "Chưa kích hoạt";
  }
  /* =======================================================
     ROLE
  ======================================================= */
  ...
  function normalizeAccount(doc) {
    ...
    const name =
      valueOf(raw, "name", "displayName", "fullName") || "Chưa cập nhật";
    ...
    return {
      ...
      name,
      email,
      campus: valueOf(raw, "campus") || "—",
      department: valueOf(raw, "department") || "—",
      phone: valueOf(raw, "phone") || "—",
      accountType: valueOf(raw, "accountType") || "customer_success",
      provider: valueOf(raw, "provider") || "—",
      role: accountRole(raw),
      status,
      statusText: statusLabel(status),
      joined: formatDate(valueOf(raw, "createdAt", "joinedAt", "dateCreated")),
      createdAtLabel: formatDateTime(raw.createdAt),
      lastActive: valueOf(raw, "lastActiveLabel", "lastActive", "lastLogin")
        ? formatDate(valueOf(raw, "lastActiveLabel", "lastActive", "lastLogin"))
        : "Chưa đăng nhập",
  ...
  function renderStats() {
    ...
    const managers = state.accounts.filter((item) =>
      /trưởng|cấp 2|manager|lead/i.test(String(item.role || "")),
    ).length;
    ...
    if (activeMeta) {
      activeMeta.textContent = total
        ? `${Math.round((active / total) * 100)}% trên tổng đội ngũ`
        : "Chưa có dữ liệu";
    }
    if (recordBadge) {
  ...
  function renderTable() {
    ...
    if (!state.filtered.length) {
      ...
      if (entries) {
        entries.textContent = "Hiển thị 0 tài khoản";
      }
      return;
    }
    body.innerHTML = state.filtered
      .map((item) => {
        const avatarBackground =
          item.status === "pending" ? "#efebe5" : "#eedcc8";
        return `
            <tr
    ...
    body.querySelectorAll("tr[data-id]").forEach((row) => {
      row.addEventListener("click", () => {
        const item = state.accounts.find(
          (account) => account.id === row.dataset.id,
        );
        openDrawer(item);
      });
    });
    ...
    body.querySelectorAll("[data-open-id]").forEach((button) => {
      button.addEventListener("click", (event) => {
        ...
        const item = state.accounts.find(
          (account) => account.id === button.dataset.openId,
        );
        openDrawer(item);
      });
    });
...
Thông tin tài khoản
  ...
  function restoreSidebar() {
    ...
    /* Tổng quan */
    if (
      path.includes("homepage-ad") ||
      path === "/" ||
      path.endsWith("/admin/")
    ) {
      page = "overview";
    } /* Tài khoản CS */ else if (
      ...
      path.includes("activity-report") ||
      path.includes("report") ||
      path.includes("bao-cao")
    ) {
      page = "reports";
    } else if (
    /* Nhật ký */
      path.includes("system-log") ||
      path.includes("logs") ||
      path.includes("nhat-ky")
    ) {
      page = "logs";
    } /* Cài đặt */ else if (
      path.includes("settings") ||
      path.includes("cai-dat")
    ) {
      page = "settings";
    }
    setActiveSidebar(page);
  ...
  function setupFirebase() {
    ...
    state.unsubscribe = database
      .collection("users")
      .where("accountType", "==", "customer_success")
      .onSnapshot(
        (snapshot) => {
          state.accounts = snapshot.docs.map(normalizeAccount);
          /* =================================================
         SORT CREATED DATE
      ================================================= */
      ...
      ================================================= */
        ...
        (error) => {
          ...
          toast("Không thể tải tài khoản CS");
        },
      );
  }
  /* =======================================================
