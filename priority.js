...
(function (global) {
  ...
  function automaticPriority(input = {}) {
    ...
    if (HIGH_ISSUES.has(issueId)) {
      level = "high";
      reason =
        "Mục yêu cầu có khả năng ảnh hưởng trực tiếp đến quyền truy cập, thanh toán hoặc hoạt động học tập.";
    } else if (LOW_ISSUES.has(issueId)) {
      level = "low";
      reason =
        "Mục yêu cầu mang tính góp ý, thông tin hoặc không cần xử lý khẩn cấp.";
    } else if (categoryId === "system") {
      level = "medium";
      reason =
        "Sự cố hệ thống cần được kiểm tra nhưng chưa có dấu hiệu khẩn cấp.";
    } else if (categoryId === "learning") {
      level = "medium";
      reason =
        "Yêu cầu liên quan đến khóa học cần được xử lý theo quy trình hỗ trợ.";
    ...
    if (URGENT_WORDS.some((word) => text.includes(word))) {
      level = "high";
      reason =
        "Nội dung có từ khóa thể hiện mức độ khẩn cấp hoặc ảnh hưởng diện rộng.";
    } else if (
      level === "low" &&
      ELEVATED_WORDS.some((word) => text.includes(word))
    ) {
      level = "medium";
      reason =
        "Nội dung có từ khóa cho thấy cần xử lý sớm hơn yêu cầu thông thường.";
    }
